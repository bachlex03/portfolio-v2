      ...
      <SheetContent className="flex flex-col items-center justify-center bg-primary gap-10">
        ...
        <Link href="/">
          <div className="flex justify-center items-center">
            <Image priority src={images.logo} width={48} height={54} className="w-[48px] h-[54px]" alt="logo" />
            <h2 className="ml-4 text-2xl font-semibold">Bale.</h2>
          </div>
        </Link>
        <nav className="flex flex-col justify-center items-center gap-8">
          {links.map((link, index) => (
            <Link href={link.path} key={index} className={`${pathname === link.path && 'border-b-2 pb-1 border-secondary'} capitalize text-xl hover:text-secondary transition-all`}>
              {link.name}
            </Link>
          ))}
          <div>
