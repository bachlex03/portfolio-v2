  ...
  return (
    <>
      <div className="xl:fixed bottom-0 left-12">
        <div className={clsx(styles.line, 'hidden xl:flex xl:flex-col justify-center items-center text-four')}>
          {icons.map((item, index) => {
            return (
              <Link key={index} href={item.path} target="_blank" className="p-[10px] text-xl hover:text-secondary hover:translate-x-2 transition duration-700">
                {item.icon}
              </Link>
            );
          })}
      ...
      <div className="xl:fixed bottom-0 right-12">
        <div className={clsx(styles.line, SFmono.variable)}>
          <p className="[writing-mode:vertical-rl] relative -left-2 text-four text-[13px] font-SFmono tracking-wider">{email}</p>
        </div>
      </div>
    </>
  );
